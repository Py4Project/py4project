...
- `bar_plot` に `legend` を追加した。`line_plot` と同じように、
  凡例に表示する名前を `x` と同じ順番・同じ数のリストで指定できる。
  省略すると列名がそのまま使われる。
  例： `bar_plot(['w2016', 'w2021'], df, legend=['2016年', '2021年'])`

### 修正

- `regression_plot` のタイトルに表示する推定式で、傾きが負のときに
  「+ -1.4984」と符号が重なっていた問題を修正した。

### その他

- `bar_plot` と `line_plot` の凡例の処理を共通化した。
  数が合わないときや `True` / `False` を渡したときのエラーが
  両方の関数で同じになる。
...
- 公開ワークフローに、タグとビルドされたバージョンが一致するかを
  確かめる手順を加えた。タグが古いコミットを指していると、
  PyPI で「400 File already exists」という分かりにくいエラーになるため。
